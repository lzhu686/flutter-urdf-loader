...
class _ExamplePageState extends State<ExamplePage> with WidgetsBindingObserver {
  ...
  void render() {
    ...
    // robot animation - only if robot is loaded
    if (robot != null) {
      for (int i = 1; i <= 6; i++) {
        ...
        robot!.trySetAngle("HP$i", lerpDouble(30, 0, ratio)! * degToRad);
        robot!.trySetAngle("KP$i", lerpDouble(90, 150, ratio)! * degToRad);
        robot!.trySetAngle("AP$i", lerpDouble(-30, -60, ratio)! * degToRad);
  ...
  Future<void> initPage() async {
    ...
    // STL - Alternative way to load 3D models
